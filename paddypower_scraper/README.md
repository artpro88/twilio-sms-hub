# 🎯 Paddy Power Pricing Scraper (LLM-powered)

An LLM-driven scraper that extracts **all events, markets and selections with
their prices** from [paddypower.com](https://www.paddypower.com).
...
```bash
# Scrape the default sport hubs, follow event links, print JSON
python -m paddypower_scraper.cli -v
...
config = ScraperConfig(fetch_backend="playwright", max_events_per_seed=30)
with PaddyPowerScraper(config) as scraper:
    events = scraper.scrape()               # crawl the seed sport hubs
...
|---|---|---|
| `PP_SEED_URLS` | football, racing, tennis… | Comma-separated pages to start from |
...
- Scraping may be **restricted by Paddy Power's Terms of Service**. Review them
  and get authorisation before scraping at scale. This tool honours
  `robots.txt` by default (`PP_RESPECT_ROBOTS=true`) and rate-limits requests.
